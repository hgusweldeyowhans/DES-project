...
# Matrix used for shifting after each round of  the keys
...
def DES(text, key, padding, isEncrypt): #Function to implement DES Algorithm.
    isDecrypt = not isEncrypt # Initializing variables required
    keys = generateKeys(key)  # Generating the keys
    # Splitting text into 8 byte blocks
    plaintext  = nSplit(text, 8)
    result = []
    # For all 8-byte blocks of text
    for block in plaintext:
        block = stringToBitArray(block) # Convert the block into bit array
        block = permutation(block, IP) # Do the initial permutation
        ...
        for i in range(16):
            ...
            expandedRightBlock = expand(rightBlock, expanision_E_table)
        ...
        result += permutation(rightBlock + leftBlock, FP)
    # Converting bit array to string
    finalResult = bitArrayToString(result)
    return finalResult
# Permutation matrix for key
def generateKeys(key): #Function to generate keys for different rounds of DES.
    ...
    return keys
def SboxSubstitution(bitArray): #Function to substitute all the bytes using Sbox.
    ...
    # Returning result
    return result
def addPadding(text): #Function to add padding according to PKCS5 standard.
    ...
    # Returning text
    return text
def removePadding(data): #Function to remove padding from plaintext according to PKCS5. 
   # Getting padding length
    paddingLength = ord(data[-1])
    # Returning data with removed padding
    return data[ : -paddingLength]
def expand(array, table):#Function to expand the array using table.
    # Returning expanded result
    return [array[element - 1] for element in table]
def permutation(array, table):#Function to do permutation on the array using table.
    # Returning permuted result
    return [array[element - 1] for element in table]
def leftShift(list1, list2, n):#Function to left shift the arrays by n.
    # Left shifting the two arrays
    return list1[n:] + list1[:n], list2[n:] + list2[:n]

def nSplit(list, n):#Function to split a list into chunks of size n.
    # Chunking and returning the array of chunks of size n
    # and last remainder
    return [ list[i : i + n] for i in range(0, len(list), n)]
def xor(list1, list2):#Function to return the XOR of two lists
    # Returning the xor of the two lists
    return [element1 ^ element2 for element1, element2 in zip(list1,list2)]
def binValue(val, bitSize): #Function to return the binary value as a string of given size.
    ...
    return binVal
def stringToBitArray(text): #Funtion to convert a string into a list of bits.
...
def bitArrayToString(array):
    ...
    return result
def encrypt(key, text, padding):#Function for DES Encryption. 
    # Adding padding if required
    if padding == True:
        text = addPadding(text)
    ciphertext = DES(text, key, padding, True) #encryption
    return ciphertext

def decrypt(key, text, padding): #Function for DES Decryption. 
    plaintext = DES(text, key, padding, False)  # Decryption
...
def main():
    ...
    key = input("Enter a key of 8 length:\n ")
    # Checking if key is valid or not
    if len(key) != 8:
        print("Invalid Key")
        return
    isPaddingRequired = (len(plaintext) % 8 != 0)  # Determining if padding is required
    ciphertext = encrypt(key, plaintext, isPaddingRequired)  # Encryption
    plaintext = decrypt(key, ciphertext, isPaddingRequired)  # Decryption
    print("Encrypted Ciphertext is : %r " % ciphertext) ## Printing result
